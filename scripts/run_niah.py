...
def main() -> int:
    ...
    ap.add_argument("--base-url", default=os.environ.get("Q38_BASE_URL", "http://127.0.0.1:30000/v1"))
    ap.add_argument("--model", default="qwen38-27b")
    ap.add_argument(
        "--tokenizer",
        default=os.environ.get(
            "Q38_TOKENIZER",
            os.path.expanduser("~/models/RadixArk/Qwen3.8-27B-NVFP4"),
        ),
    )
    ...
    for depth in depths:
        ...
        status, body, elapsed = post_chat(comp_url, comp_payload, timeout=7200)
        text = ""
        if status == 200 and body.get("choices"):
            ...
            status, body, elapsed = post_chat(endpoint, chat_payload, timeout=7200)
            if status == 200 and body.get("choices"):
