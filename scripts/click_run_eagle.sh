#!/usr/bin/env bash
# Click-run official EAGLE 3/1/4 (in-checkpoint MTP) + pinned image + canary.
set -euo pipefail
...
docker pull "$IMAGE"
MODEL_DIR="$CKPT" IMAGE="$IMAGE" MEM_FRACTION="$MEM_FRACTION" \
  bash "$(dirname "$0")/serve.sh" eagle
ready=0
...
python3 "$(dirname "$0")/run_semantic_gate.py" --base-url http://127.0.0.1:30000 --output /tmp/qwen38-sglang-eagle-semantic.json
python3 - <<'PY'
import json
d=json.load(open("/tmp/qwen38-sglang-eagle-semantic.json"))
