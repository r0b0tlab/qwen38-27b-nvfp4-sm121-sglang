...
IMAGE="${IMAGE:-qwen38-27b-sglang-dflash2-sm121:0.2.0}"
...
PY
# Build the overlay image if missing (pinned day-0 base + PR #35371 @ c14312a).
docker image inspect "$IMAGE" >/dev/null 2>&1 || docker build -f docker/Dockerfile.dflash2 -t "$IMAGE" docker/
...
python3 "$(dirname "$0")/run_semantic_gate.py" --base-url http://127.0.0.1:30000 --output /tmp/qwen38-dflash2-semantic.json
python3 - <<'PY'
import json
