...
class DFlash2DraftModel(DFlashDraftModel):
    ...
    def compute_candidates(
        ...
        if not is_dense_head_weight(weight):
            # Quantized head (e.g. ModelOpt W4A16 NVFP4 / GGUF packed): the raw
            # weight cannot feed a dense matmul. Run the head's own dequantizing
            # kernel -- the same call LogitsProcessor._get_logits makes -- and
            # take top-k on the real logits. Mirrors the worker's
            # _greedy_sample_from_quantized_head; tp=1 only, selector stays
            # eager (the worker screen keeps it out of the draft CUDA graph).
            if get_parallel().tp_size != 1:
                raise RuntimeError(
                    "DFlash2 selector with a quantized target lm_head is only "
                    f"supported at tp=1, got tp_size={get_parallel().tp_size}."
                )
            num_tokens = int(hidden.shape[0])
            num_org = int(getattr(self.lm_head, "org_vocab_size", 0)) or None
            ids_parts, vals_parts = [], []
            chunk = 256
            for start in range(0, num_tokens, chunk):
                end = min(num_tokens, start + chunk)
                logits = self.lm_head.quant_method.apply(
                    self.lm_head, hidden[start:end], None
                )
                if num_org is not None and logits.shape[-1] > num_org:
                    logits = logits[:, :num_org]
                vals, ids = _radix_topk(logits.float(), k)
                ids_parts.append(ids)
                vals_parts.append(vals)
            return (
                torch.cat(ids_parts, dim=0).long(),
                self._transform_unary_logits(torch.cat(vals_parts, dim=0)),
            )
